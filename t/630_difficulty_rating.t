...
is($difficulty->rating_version, '2.7', 'difficulty rating records method version');
...
like($difficulty->summary, qr/v2\.7/, 'summary includes rating version');
like($difficulty->summary, qr/X-Wing/, 'summary includes highest strategy');

my $hash = $difficulty->as_hash;
is($hash->{rating_version}, '2.7', 'as_hash includes rating version');
...
is(
    $difficulty->strategy_score('Grouped L1-Wing'),
    9,
    'Grouped L1-Wing has an active difficulty score',
);

is(
    $difficulty->strategy_score('AIC'),
    10,
    'AIC has an active difficulty score',
);


is(
    $difficulty->strategy_score('Grouped AIC'),
    10,
    'Grouped AIC has an active difficulty score',
);

is(
    $difficulty->strategy_score('Jellyfish'),
    8,
    'Jellyfish has an active difficulty score',
);
...
is(
    $difficulty->strategy_score('Digit Forcing Chains'),
    11,
    'Digit Forcing Chains has an active difficulty score',
);
