package Sudoku::Strategy::Jellyfish;
...
sub apply {
    ...
    for my $pattern (find_fish_patterns($grid, 4)) {
        ...
        for my $target (@{ $pattern->{targets} }) {
            ...
            push @deductions, Sudoku::Deduction->new(
                ...
                reason   => sprintf(
                    ...
                    . 'Those four base units must place %d within the same '
                    . 'four cover units, so %s cannot contain %d.',
                    $value,
                    $cover_name,
                    $cover_text,
                    $base_name,
                ...
                explanation => sprintf(
                    'Remove candidate %d from %s. Jellyfish cells: %s.',
                    $value,
                    cell_label($target),
                    $corner_text,
                ),
