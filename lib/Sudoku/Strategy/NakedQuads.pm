package Sudoku::Strategy::NakedQuads;
...
sub apply {
    ...
    for my $unit ($self->_units($grid)) {
        for my $subset (Sudoku::Subset->naked_subsets($unit->{cells}, 4)) {
            ...
            for my $cell (grep { !$_->value && !$member{ _cell_key($_) } } @{ $unit->{cells} }) {
                for my $value (@{ $subset->{values} }) {
                    ...
                    my $reason = sprintf(
                        'Cells %s form a Naked Quad {%s} in %s %d, so no other cell in that unit can contain %d.',
                        $locations,
                        $values,
                        $unit->{type},
                        $unit->{index} + 1,
